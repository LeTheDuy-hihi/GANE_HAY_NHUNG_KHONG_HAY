...
class Panel:
    ...
    def __init__(self, x, y, w, h, title="", border_color=None):
        self.rect = pygame.Rect(x, y, w, h)
        self.title = title
        self.border_color = border_color or UI_BORDER
        self._pulse = 0.0

    def draw(self, screen, font=None):
        self._pulse += 0.03
        # Background
        pygame.draw.rect(screen, UI_PANEL, self.rect, border_radius=6)
        # Border
        ...
        for corner in [(self.rect.topleft, 1, 1), (self.rect.topright, -1, 1),
            ...
            pos, dx, dy = corner
            x, y = pos
            pygame.draw.line(screen, UI_HIGHLIGHT, (x, y), (x + size * dx, y), 2)
            pygame.draw.line(screen, UI_HIGHLIGHT, (x, y), (x, y + size * dy), 2)

        # Title
        if self.title and font:
            txt = font.render(self.title, True, UI_HIGHLIGHT)
            screen.blit(txt, (self.rect.x + 15, self.rect.y + 8))
            # Line under title
            pygame.draw.line(screen, (*UI_BORDER, 100),
                             (self.rect.x + 10, self.rect.y + 32),
                             (self.rect.right - 10, self.rect.y + 32), 1)
